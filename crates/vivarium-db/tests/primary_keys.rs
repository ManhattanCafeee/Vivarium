...
#[tokio::test]
async fn unset_string_key_is_rejected_with_a_clean_message() {
    let pool = pool().await;
    ...
    .await
    .expect_err("a String key cannot be generated by the database");

    let message = error.to_string();
    assert!(
        message.contains("not generated by the database"),
        "the error must explain why the key was rejected, got {message}"
    );
    assert!(
        !message.contains("  "),
        "the error must not contain a run of spaces, got {message}"
    );

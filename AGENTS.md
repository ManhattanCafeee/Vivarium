...
| `vivarium-web` | axum layer: `ApiResponse`/`ApiError` envelope, `Texts` catalog, `Varser` family, JWT/session, `Cache` layer, OpenAPI helpers | **none — standalone** (optional `sqlx` feature for `conflict_from_db`) |
...
- **Feature gating:** driver impls behind `#[cfg(feature = "sqlite")]` etc.; facade uses `dep:` optional deps. `vivarium-db` has `default = []` (no driver); `vivarium-core` has `chrono`/`uuid`/`utoipa`; `vivarium-web` has `validation-validator` (default), `validation-garde`, `utoipa`, `utoipa-ui`, `sqlx`; `vivarium-rs` default = web+config+db+sqlite+postgres (no mysql) and passes `utoipa`/`utoipa-ui`/`validation-garde` through
...
- `crates/vivarium-web/src/{jwt,session,token,password,cache,serve}.rs` — auth, hashing, Cache-Control, serving
